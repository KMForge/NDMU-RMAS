...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
    <div class="p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-800">
            ...
            @foreach($pendingActions as $item)
                <div class="p-4 hover:bg-slate-50/80 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ $item['route'] }}"
                           class="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white font-medium text-xs rounded-lg shadow-xs transition-colors inline-flex items-center gap-1.5">
    ...
    @else
        <div class="p-8 text-center text-slate-500">
            ...
            <p class="text-sm font-medium text-slate-700">No pending academic actions</p>
            <p class="text-xs text-slate-400 mt-0.5">All official forms under your academic purview are currently up to date.</p>
        </div>
    @endif
</div>
