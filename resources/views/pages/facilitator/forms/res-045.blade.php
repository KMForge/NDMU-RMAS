...
<div x-show="activeOfficialForm === 'RES-045'" x-cloak>
    <x-student-official-form code="RES-Form-045" title="Certificate of Language Editing" guidebook-page="131">
        ...
        <p>This is to certify that the research paper entitled:</p><input value="{{ $officialFormInstance?->group?->researchGroup?->title }}" class="w-full text-center" aria-label="Research title" readonly><p>to be submitted by the student researcher/s whose name/s appear below:</p><div class="grid gap-3 md:grid-cols-2">@for ($i=1; $i<=4; $i++)<label class="flex gap-2"><span>{{ $i }}.</span><input value="{{ $officialFormInstance?->group?->members?->values()?->get($i - 1)?->student?->name }}" class="flex-1" readonly></label>@endfor</div>
        ...
        <div class="grid gap-8 pt-8 text-center md:grid-cols-2"><x-official-signature-field name-field="res_045_editor_printed_name" label="Language Editor" /><label>Date Edited:<input type="date" name="payload[date]" value="{{ $payload['date'] ?? '' }}" class="w-full"></label></div>
