<div x-show="activeOfficialForm === 'RES-026'" x-cloak><x-student-official-form code="RES-Form-026" title="Research Title Approval" guidebook-page="101">
        ...
        <fieldset><legend class="mb-2 font-bold">Name of Student/s:</legend><div class="grid grid-cols-1 gap-3 md:grid-cols-2">@for ($i = 1; $i <= 4; $i++)<label class="flex gap-2"><span>{{ $i }}.</span><input type="text" autocomplete="name" class="min-w-0 flex-1" name="students[]"></label>@endfor</div></fieldset>
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-4 pt-3 text-center"><div class="border-b border-[#173c30] pb-1 font-bold">Name</div><div class="border-b border-[#173c30] pb-1 font-bold">Signature</div><div class="col-span-2 text-left font-bold">Panel of Examiners:</div>@foreach (['Chairman','Member 1','Member 2'] as $role)<label class="flex items-center gap-2 text-left italic"><span class="shrink-0">{{ $role }}:</span><input type="text" name="panel_names[]" class="min-w-0 flex-1"></label><x-official-signature-field :label="$role.' signature'" />@endforeach</div>
        <div class="grid grid-cols-2 gap-8 pt-8 text-center"><x-official-signature-field name-field="program_coordinator_name" label="Program Coordinator" /><x-official-signature-field name-field="college_dean_name" label="College Dean" /></div>
