    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="role-assignment-title">
        ...
        <form wire:submit="saveUserRoles" class="relative z-10 w-full max-w-xl rounded-3xl bg-white shadow-2xl">
            ...
            <div class="max-h-[60vh] space-y-2 overflow-y-auto p-6">
                <label for="assigned-user-type" class="block rounded-xl border border-gray-100 p-4">
                    <span class="mb-2 block text-xs font-bold uppercase tracking-wide text-gray-600">Primary user type</span>
                    <select id="assigned-user-type" wire:model="assignedUserType" class="w-full rounded-xl border-gray-200 text-sm focus:border-[#0e5c3a] focus:ring-[#0e5c3a]">
                        @foreach (\App\Enums\UserType::cases() as $type)
                            <option value="{{ $type->value }}">{{ $type->label() }}</option>
                        @endforeach
                    </select>
                    <span class="mt-2 block text-[10px] text-gray-500">This classifies the account; roles below determine actual access.</span>
                    @error('assignedUserType') <span class="mt-1 block text-xs font-semibold text-red-600">{{ $message }}</span> @enderror
                </label>
                @foreach ($rolesList as $availableRole)
                    <label class="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-gray-100 p-4 hover:bg-gray-50">
                        <span>
                            ...
                            <span class="mt-1 block text-[10px] text-gray-500">Reusable access role &middot; {{ $availableRole['permissions_count'] }} permissions</span>
                        </span>
