...
- **Thesis Adviser**: Receives adviser invitation requests, accepts or declines requests, and views assigned groups and students.
...
- Route middleware enforces `auth`, `verified`, `active`, and Spatie permission checks.
- Domain actions enforce facilitator ownership check on classes and groups, throwing `AuthorizationException` (403) on mismatch to prevent IDOR attacks.
- Adviser response endpoint validates that the authenticated user matches `adviser_id` on the pending request.
- Concurrency locks (`Cache::lock` and `DB::transaction` with `lockForUpdate`) protect against race conditions when adding 5th members or sending duplicate requests.
...
| `app/Http/Controllers/Adviser/ResearchGroupAdviserRequestController.php` | NEW | Controller for adviser request accept/decline responses. |
...
| `app/Http/Controllers/Adviser/DashboardController.php` | MODIFY | Updated dashboard to pass pending adviser requests and assigned groups. |
...
| `tests/Feature/Classes/ResearchClassGroupWorkflowTest.php` | MODIFY | Reactivated and expanded test suite for all Phase 12 features. |
| `docs/PHASE_12_RESEARCH_GROUPS_AND_ADVISER_ASSIGNMENT.md` | NEW | Technical documentation for Phase 12. |
...
- **Phase 10, 11 & 12 Class Tests**: 31 tests passed cleanly (`100%` pass rate, `0` failures).
- **Phase 12 Group Workflow Tests**: 16 focused tests passed (`0` failures).
- **Pint Formatting**: Applied cleanly across all PHP files.
- **Vite Build**: Compiled client assets successfully with zero errors.
