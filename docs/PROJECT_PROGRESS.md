...
| Latest completed feature phase | `Phase 16 — Consultation Records` |
| Current active feature phase | `Phase 17 — Revision Tracker (planned)` |
...
| Phase 17 | Revision Tracker | Completed | Implemented modernized group-owned revision requests (`ResearchClassGroup`), 1-to-1 database unique index on `source_document_review_id`, group leader start/submit authority, bidirectional decision correction reconciliation (`Cancelled` invalidation vs auto-regeneration), current adviser resolution with blocking findings enforcement (`severity` in `['revision', 'critical']`), controlled reopening (`Resolved -> Submitted` with mandatory reason), due date management, and read-only facilitator supervision monitoring. Verified by Phase 17 test suite (8 tests, 28 assertions, 0 failures), Pint formatting, and Vite production frontend build. Documentation added to `docs/PHASE_17_REVISION_TRACKER.md`. |
