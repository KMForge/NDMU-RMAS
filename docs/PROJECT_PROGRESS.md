...
| Latest completed feature phase | `Phase 24 — Audit Logs` |
| Current active feature phase | `Phase 25 — Reports and Analytics (Implemented; repository-wide regression gate pending)` |
...
| Phase 19 | Official Research Forms | Implemented with External Dependencies | The locked RES-026 Title Presentation workflow now uses an approved Title Proposal document as its prerequisite and exact source, exactly three manually entered titles, Phase 21 scheduling/history, authoritative Chairperson/Member assignments, exact-version Phase 20 signatures, explicit Coordinator and Dean actions, canonical-title finalization, and a separate explicit Phase 18 milestone action. External institutional questions for other forms remain unchanged. See `PHASE_19_OFFICIAL_RESEARCH_FORMS.md` and `OFFICIAL_RESEARCH_WORKFLOW.md`. |
...
| Phase 25 | Reports and Analytics | Implemented; Verification Pending | Added permission-driven CEAC reports for Administrator, Dean, and facilitator-owned classes; 11 catalogued reports; validated shared filters; bounded HTML, streamed CSV, and PDF output; scope-aware caching; formula-injection protection; and centralized report audit events. Seven focused tests pass with 29 assertions. Pint, report route inspection, migration status, Laravel config/route/view caches, production Vite build, and diff checks pass. The required full-suite gate is not green because two existing welcome-page tests conflict with the current landing-page redesign (421 passed, 24 skipped, 2 failed in the full run). Composer validation was not run because Composer is unavailable on PATH. See `PHASE_25_REPORTS_AND_ANALYTICS.md`. |
