...
> **Baseline note:** Commit `f701324` remains the functional rebuild baseline. Phases 10 through 18 are complete. Phase 18 replaced the disposable legacy percentage rows with group-owned, weighted, auditable milestones while preserving all user accounts and local MySQL portability.
...
| Phase 19 | Official Research Forms | In Progress | Backend foundation remains active with portable catalog/version/actor-assignment persistence, driver-guarded RLS, Spatie form permissions, and action-aware policies. The generic authorization cleanup now makes RES-040/041 `endorse` and `receive` explicit, enforces ordered form/action transitions, requires exact same-scope actor types for class-owned initiation, separates `initiated_by` draft ownership from academic authority, and fails closed for unknown actions. Focused verification: `OfficialFormBackendTest` 16 tests/36 assertions and `FormPermissionsTest` 5 tests/16 assertions, all passing. Official Forms suite: 33 tests, 231 assertions, 0 failures. Pint passed, the Vite production build passed (58 modules), and all migrations are applied. Full regression was also run: 289 tests, 1,256 assertions, 257 passed, 9 failed, and 23 skipped; the failures are outside this cleanup in legacy admin/dashboard expectations, intentionally disabled Phase 20 signature endpoints, and an existing student milestone assertion. Phase 19 remains in progress while per-form workflows are implemented. |
