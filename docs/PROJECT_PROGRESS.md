...
| Phase 19 | Official Research Forms | In Progress | Implemented backend foundation, portable migrations (definitions, instances, versions, actor assignments), driver-guarded RLS security, Spatie Form Permissions catalog (58 permissions), default specialist roles (`research-instructor`, `language-editor`, `technical-editor`, `instrument-validator`), domain actions (`CreateOfficialFormInstance`, `SubmitOfficialFormVersion`, `AssignOfficialFormActor`, `ApproveOfficialForm`, `CertifyOfficialForm`), versioning transaction locks, and `OfficialFormInstancePolicy`. Verified by 24 tests, 214 assertions, 0 failures, Pint, and production Vite build. |
