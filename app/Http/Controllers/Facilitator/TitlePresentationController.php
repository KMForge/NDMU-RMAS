...
{
    public function store(Request $request, OfficialFormInstance $instance, ScheduleTitlePresentation $schedule): RedirectResponse
    {
        ...
        return $this->mutate(fn () => $schedule->handle($request->user(), $instance, (int) $validated['room_id'], Carbon::parse($validated['starts_at']), Carbon::parse($validated['ends_at'])), 'Title Presentation scheduled.');
    }
    ...
    public function complete(Request $request, TitlePresentation $presentation, CompleteTitlePresentation $complete): RedirectResponse
    {
        return $this->mutate(fn () => $complete->handle($request->user(), $presentation), 'Title Presentation marked completed.');
    }
